...
class TransactionObserver
{
    ...
    /**
     * Flag to prevent infinite loops when creating automated investment transactions
     */
    private static bool $processingAutoInvestment = false;
    ...
    public function creating(Transaction $transaction): bool
    {
        ...
        // Validate against balance checkpoints (use update-style validation to avoid timing/state differences)
    ...
    {
        // Create automated investment transactions for standard transactions
        if (self::$processingAutoInvestment) {
            return;
        }

        try {
            self::$processingAutoInvestment = true;
            $service = new \App\Services\AutoInvestmentService();
            $service->handleTransaction($transaction);
        } finally {
            self::$processingAutoInvestment = false;
        }
    }
