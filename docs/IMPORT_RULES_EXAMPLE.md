...
### Example: Convert "PropertyCo - Rental Income" to a Transfer
...
$importAccount = \App\Models\AccountEntity::where('name', 'IncomeAccount')->first();
$PropertyCo = \App\Models\AccountEntity::where('name', 'LIKE', '%PropertyCo%')->first(); // Find the Jan Forster account

// Create the rule
\App\Models\TransactionImportRule::create([
    'user_id' => $user->id,
    'account_id' => $importAccount->id, // Apply only when importing to this account
    'description_pattern' => 'PropertyCo - Rental Income',
    'use_regex' => false,
    'action' => 'convert_to_transfer',
    'transfer_account_id' => $PropertyCo->id,
...
```php
// Skip JAN FORSTER deposits when importing PropertyCo's account
\App\Models\TransactionImportRule::create([
    'user_id' => $user->id,
    'account_id' => $janForsterAccount->id, // Only applies to PropertyCoaccount imports
